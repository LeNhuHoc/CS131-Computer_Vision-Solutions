...
def simple_descriptor(patch):
    ...
    if std > 0.0:
        ...
        feature = patch - mean
    feature = feature = feature.reshape(-1)
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    for i in range(N):
        dist = dists[i,:]
        ratio = np.min(dist)/(np.partition(np.sort(dist),2)[1])
        if ratio <= threshold:
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    Use RANSAC to find a robust affine transformation
        ...
        3. Compute inliers using the given threshold
        4. Repeat & Keep the largest set of inliers
    ...
    for i in range(n_iters):
        ...
        H=np.linalg.lstsq(p2,p1,rcond=None)[0]
        H[:,2]=np.array([0,0,1])
        # Compute inliers using given threshold
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    # ## Code reference
    # #They do step 4 before doing step 3
