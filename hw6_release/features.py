...
class LDA(object):
    ...
    def fit(self, X, y):
        ...
        e_vals, e_vecs = scipy.linalg.eig(scatter_between,scatter_within) #Reference 1
        # e_vals, e_vecs = scipy.linalg.eig(np.linalg.inv(scatter_within).dot(scatter_between)) #Reference 2
        idx = np.argsort(-e_vals)
        e_vals = e_vals[idx]
        e_vecs = e_vecs[:,idx]
        pass
    ...
    def _between_class_scatter(self, X, y):
        ...
        Reference: Our result 'scatter_between' is the covatiance matrix of X 
        where we replaced every example labeled i with mu_i
